...
namespace CedricZiel\Simplebase\Framework\Utility;
...
/**
 * Helper with registration of simplebase plugins
 *
 * @package CedricZiel\Simplebase\Framework\Utility
 */
...
tt_content.list.20.'.$pluginSignature.' {
	userFunc = CedricZiel\\Simplebase\\Framework\\Bootstrap->run
...
tt_content.'.$pluginSignature.' {
	...
	20 {
        ...
        $GLOBALS['TYPO3_CONF_VARS']['EXTCONF']['simplebase']['extensions'][$extensionName]['plugins'][$pluginName]['pluginType'] = $pluginType;
...
'.$pluginContent, 'defaultContentRendering');
    ...
    public static function registerPlugin($extensionName, $pluginName, $pluginTitle, $pluginIconPathAndFilename = null)
    {
        ...
        $pluginType = isset($GLOBALS['TYPO3_CONF_VARS']['EXTCONF']['simplebase']['extensions'][$extensionName]['plugins'][$pluginName]['pluginType'])
            ? $GLOBALS['TYPO3_CONF_VARS']['EXTCONF']['simplebase']['extensions'][$extensionName]['plugins'][$pluginName]['pluginType']
            : 'list_type';
    ...
    public static function registerModule(
        ...
        $defaultModuleConfiguration = array(
            ...
            'labels' => '',
            'extRelPath' => ExtensionManagementUtility::extRelPath($extensionKey).'Classes/',
        );
        ...
        $moduleConfiguration['configureModuleFunction'] = array(
            ExtensionManagementUtility::class,
            'configureModule',
        );
        $GLOBALS['TBE_MODULES']['_configuration'][$moduleSignature] = $moduleConfiguration;
        if (!is_array($GLOBALS['TYPO3_CONF_VARS']['EXTCONF']['simplebase']['extensions'][$extensionName]['modules'][$moduleSignature])) {
            $GLOBALS['TYPO3_CONF_VARS']['EXTCONF']['simplebase']['extensions'][$extensionName]['modules'][$moduleSignature] = array();
        }
        foreach ($controllerActions as $controllerName => $actions) {
            $GLOBALS['TYPO3_CONF_VARS']['EXTCONF']['simplebase']['extensions'][$extensionName]['modules'][$moduleSignature]['controllers'][$controllerName] = array(
                'actions' => GeneralUtility::trimExplode(',', $actions),
            );
        }
        ExtensionManagementUtility::addModule($mainModuleName, $subModuleName, $position);
    }
    ...
    {
        if (!is_array($GLOBALS['TYPO3_CONF_VARS']['EXTCONF']['simplebase']['typeConverters'])) {
            $GLOBALS['TYPO3_CONF_VARS']['EXTCONF']['simplebase']['typeConverters'] = array();
        }
        $GLOBALS['TYPO3_CONF_VARS']['EXTCONF']['simplebase']['typeConverters'][] = $typeConverterClassName;
    }
    ...
    protected static function checkVendorNameFormat($vendorName, $extensionName)
    {
        if (preg_match('/^[A-Z]/', $vendorName) !== 1) {
            GeneralUtility::deprecationLog('The vendor name from tx_'.$extensionName.' must begin with a capital letter.');
        }
    }
